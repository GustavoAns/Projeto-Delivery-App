...
export default function Checkout() {
  const { itensCarrinho, carrinho, setItensCarrinho, setCarrinho } = useContext(AppContext);
  const [formValues, setFormValues] = useState({ deliveryAddress: '', deliveryNumber: '' });
  ...
  return (
    <div>
            ...
            carrinho.map(({id, name, price }, index) => {
                return (
                  ...
                  <tr key={ id }>
                    ...
                    >{ itensCarrinho[index].quantity }</td>
                    <td
                    data-testid={ `customer_checkout__element-order-table-unit-price-${index}` }
                    >{ price }</td>
                    <td
                    data-testid={ `customer_checkout__element-order-table-sub-total-${index}` }
                    >{ (itensCarrinho[index].quantity  * price).toFixed(2) }</td>
                  </tr>
                  <button 
      ...
      <div data-testid="customer_checkout__element-order-total-price">
        ...
        {
            carrinho &&
            (carrinho.reduce((acc, {price, quantity}, index) => {
                let total = 0
